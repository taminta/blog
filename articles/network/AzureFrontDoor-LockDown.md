...
## <a href="#frontdoor-lockdown-overview">Azure Front Door におけるセキュアな構成</a>

Azure Front Door は上記の通り多くの機能を要しています。その中で、Azure Front Door の WAF (Web アプリケーション ファイアウォール) を利用いただければ、バックエンドに配置した仮想マシンや App Service 上のアプリケーションを、一般的な悪用や脆弱性から保護する構成がご利用いただけます。
<br>
...
<br>
そのため、例えばバックエンドに仮想マシンを配置されている構成であれば、NSG のサービス タグで「AzureFrontDoor.Backend」からの HTTP/HTTPS 通信のみ許可することで、Azure Front Door のパブリック IP アドレス以外からの HTTP/HTTPS 接続を許可しない構成が可能です。

<br>

<span id="frontdoor-lockdown-fdid"></span>

## <a href="#frontdoor-lockdown-fdid">Azure Front Door の固有のリソース ID</a>
...
## <a href="#frontdoor-lockdown-sample">アプリケーション上のサンプル (IIS)</a>

仮想マシン上で動作するアプリケーションにおける、IIS における設定例としては下記の設定があります。お客様が構成されたアプリケーションに適した形でご設定ください。
<br>
なお、仮想マシン上で動作するアプリケーションにおいては、お客様の構成範囲となりますため、Azure サポート担当では具体的なご支援ができない点はご理解いただきますようお願い申し上げます。
