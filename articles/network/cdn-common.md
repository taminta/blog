...
接続制限という観点でリージョンを指定されたい場合は、[国/地域別の Azure CDN コンテンツの制限](https://docs.microsoft.com/ja-jp/azure/cdn/cdn-restrict-access-by-country) の機能をご利用ください。<br>
<br>
...
Azure CDN のエンドポイントに設定いただいたカスタムドメインを、別のエンドポイントに移行する場合は、一度既存のカスタムドメインの設定を削除いただいた上で移行していただく必要があります。<br>
<br>
...
設定後、意図した動作とならない場合は、一度 [キャッシュ削除 (Purge)](https://docs.microsoft.com/ja-jp/azure/cdn/cdn-purge-endpoint) を実行いただき、動作をご確認ください。<br>
<br>
